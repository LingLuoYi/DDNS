...
public class CloudflareDns {
    ...
    public static void dns(Map<String,String> header,String domain, String subDomain,String urlIp,String zoneId) {
        try {
            String endDomain = domain;
            if (subDomain != null && !"".equals(subDomain)) {
                endDomain = subDomain + "." + domain;
            }
            if (zoneId == null || "".equals(zoneId)) {
                System.out.println("获取zoneId失败，请检查域名是否正确并且添加了解析记录");
                return;
            }
            //获取域记录
            String domainData = HttpUtil.doGet(URL + "/zones/" + zoneId + "/dns_records?name=" + endDomain, header);
//            System.out.println("domainData--->" + domainData);
            ...
            for (Map result : recordsResults) {
                if (endDomain.equals(result.get("name"))) {
                    ...
                    if (ip.equals(result.get("content"))) {
                        ...
                        params.put("type", "A");
                        params.put("name", endDomain);
                        params.put("content", ip);
                        params.put("ttl", 600);
                        params.put("proxied", false);
                        String recordData = HttpUtil.doPut(URL + "/zones/" + zoneId + "/dns_records/" + result.get("id"), new StringEntity(JSONObject.toJSONString(params), StandardCharsets.UTF_8), header);
