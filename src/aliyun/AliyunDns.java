...
public class AliyunDns {
    ...
    public static void dns(String keyIp, String secret, String[] domains, String[] subDomains, String ipUrl) {
            ...
            for (String subDomain : subDomains) {
                if (subDomain.contains(domain)) {
                    ...
                    try {
                        ...
                        if (!wip.equals(value)) {
                            System.out.println("域:" + subDomain + ",检测到本地外网IP与阿里云配置IP不相符，将替换,检测时间：" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
                            String b = commonRpc.gain_ip(RecordId, subDomain, wip);
                            System.out.println("成功，请求码：" + b);
                            s = true;
                        } else {
                            System.out.println("域:" + subDomain + ",当前外网IP与阿里云配置相同，检测时间：" + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date()));
                            s = false;
                        }
                        System.out.println("---------------------------------------");
                        System.out.println();
